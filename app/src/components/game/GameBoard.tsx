...
function statusLabel(status: GameState["status"]): {
  label: string;
  color: string;
} {
...
export function GameBoard({ game, gamePDA, settleOnly }: Props) {
  ...
  const [holderPriceAtReceive, setHolderPriceAtReceive] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    (erProgram.account as any).playerAccount
      .fetch(holderPDA)
      .then((acc: any) =>
        setHolderPriceAtReceive(acc.priceAtReceive.toNumber())
      )
      .catch(() => {});
  ...
  const liveScoreBp =
    isActive && holderPriceAtReceive && solPriceNow
      ? calcScorePreview(
    ...
    try {
      ...
      if (!alreadyCommitted) {
        ...
        while (Date.now() < deadline) {
          try {
            erGame = await (erProgram.account as any).gameState.fetch(gamePDA);
            console.log("ER STATUS:", erGame.status);
            console.log("ER PDA:", gamePDA.toBase58());
            console.log({
              status: erGame.status,
              endsAt: erGame.endsAt.toString(),
              now: Math.floor(Date.now() / 1000),
              diff: erGame.endsAt.toNumber() - Math.floor(Date.now() / 1000),
            });
          } catch (e) {
            /* retry */
            console.error("ER FETCH FAILED", e);
          }
        ...
        if (!erGame || !("ended" in erGame.status)) {
          throw new Error(
            "Game has not ended on ER yet — try again in a moment"
          );
        }
        setSettleError(null);
        ...
        if (txInfo?.meta?.err) {
          throw new Error(
            "commitAndSettle failed on ER: " + JSON.stringify(txInfo.meta.err)
          );
        }
  ...
  if (settleOnly) {
    return (
      <div className="space-y-2">
        {settleError && (
          <div className="text-red-400 text-xs">{settleError}</div>
        )}
        <button
  ...
  //   sessionExists,
  //   isExpired,
  //   sessionPDA: sessionTokenPDARef.current?.toBase58(),
  //   tempKey: tempKeypairRef.current?.publicKey?.toBase58(),
  // });
        ...
        <div className="flex items-center gap-3">
          <span
            className={`text-xs font-bold uppercase tracking-widest px-2 py-1 rounded border ${
              isEnded ? "text-red-400 border-red-800" : color
            } border-current`}
          >
            {isEnded ? "ENDED" : label}
          </span>
